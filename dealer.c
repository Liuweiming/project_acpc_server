...
static void printUsage(FILE *file, int verbose) {
  fprintf(file,
          "usage: dealer matchName gameDefFile #Hands rngSeed p1name p2name "
          "... [options]\n");
  ...
  fprintf(file,
          "  --start_timeout [milliseconds] maximum time to wait for players "
          "to connect\n");
...
static void initErrorInfo(const uint32_t maxInvalidActions,
                          ...
                          const uint64_t maxUsedMatchMicros, ErrorInfo *info) {
  int s;
...
static int sendPlayerMessage(const Game *game, const MatchState *state,
  ...
  /* log the message */
  if (!quiet) {
    fprintf(stderr, "TO %d at %zu.%.06zu %s", seat + 1, sendTime->tv_sec,
            sendTime->tv_usec, line);
  }
...
static int readPlayerResponse(const Game *game, const MatchState *state,
                              ...
                              ErrorInfo *errorInfo, ReadBuf *readBuf,
  ...
  while (1) {
    ...
    if (getLine(readBuf, MAX_LINE_LEN, line, errorInfo->maxResponseMicros) <=
        0) {
    ...
    /* log the response */
    if (!quiet) {
      fprintf(stderr, "FROM %d at %zu.%06zu %s", seat + 1, recvTime->tv_sec,
              recvTime->tv_usec, line);
    }
    ...
    /* get the action */
    if (line[c++] != ':' || (r = readAction(&line[c], game, action)) < 0) {
...
static int processTransactionFile(const Game *game, const int fixedSeats,
  ...
  while (fgets(line, MAX_LINE_LEN, file)) {
    ...
    if (sscanf(&line[c], " %" SCNu32 " %zu.%06zu %zu.%06zu%n", &h,
               &sendTime.tv_sec, &sendTime.tv_usec, &recvTime.tv_sec,
               &recvTime.tv_usec, &r) < 4) {
    ...
    /* check for any timeout issues */
    s = playerToSeat(game, *player0Seat, currentPlayer(game, &state->state));
    ...
    if (stateFinished(&state->state)) {
      ...
      for (s = 0; s < game->numPlayers; ++s) {
        totalValue[s] += valueOfState(game, &state->state,
                                      seatToPlayer(game, *player0Seat, s));
      }

      /* move on to next hand */
      if (setUpNewHand(game, fixedSeats, handId, player0Seat, rng, errorInfo,
                       &state->state) < 0) {
        return -1;
      }
    }
...
static int logTransaction(const Game *game, const State *state,
                          const Action *action, const struct timeval *sendTime,
                          const struct timeval *recvTime, FILE *file) {
  int c, r;
  char line[MAX_LINE_LEN];
  ...
  r = snprintf(&line[c], MAX_LINE_LEN - c, " %" PRIu32 " %zu.%06zu %zu.%06zu\n",
...
static int checkVersion(const uint8_t seat, ReadBuf *readBuf) {
  ...
  if (sscanf(line, "VERSION:%" SCNu32 ".%" SCNu32 ".%" SCNu32, &major, &minor,
             &rev) < 3) {
    fprintf(stderr, "ERROR: invalid version string %s", line);
    return -1;
  }

  if (major != VERSION_MAJOR || minor > VERSION_MINOR) {
    fprintf(stderr,
            "ERROR: this server is currently using version %" SCNu32 ".%" SCNu32
            ".%" SCNu32 "\n",
...
static int addToLogFile(const Game *game, const State *state,
                        ...
                        const uint8_t player0Seat, char *seatName[MAX_PLAYERS],
                        FILE *logFile) {
  int c, r;
  uint8_t p;
  char line[MAX_LINE_LEN];
  ...
  /* add the values */
  for (p = 0; p < game->numPlayers; ++p) {
    r = snprintf(&line[c], MAX_LINE_LEN - c, p ? "|%.6f" : ":%.6f", value[p]);
    if (r < 0) {
  ...
  for (p = 0; p < game->numPlayers; ++p) {
    r = snprintf(&line[c], MAX_LINE_LEN - c, p ? "|%s" : ":%s",
...
static int printInitialMessage(const char *matchName, const char *gameName,
  ...
  c = snprintf(line, MAX_LINE_LEN,
               ...
               info->maxResponseMicros / 1000, info->maxUsedHandMicros / 1000,
...
static int printFinalMessage(const Game *game, char *seatName[MAX_PLAYERS],
  ...
  for (s = 0; s < game->numPlayers; ++s) {
    r = snprintf(&line[c], MAX_LINE_LEN - c, s ? "|%.6f" : ":%.6f",
                 totalValue[s]);
    if (r < 0) {
  ...
  /* add the player names */
  for (s = 0; s < game->numPlayers; ++s) {
    r = snprintf(&line[c], MAX_LINE_LEN - c, s ? "|%s" : ":%s", seatName[s]);
    if (r < 0) {
      fprintf(stderr, "ERROR: log message too long\n");
      return -1;
    }
...
static int gameLoop(const Game *game, char *seatName[MAX_PLAYERS],
                    ...
                    ReadBuf *readBuf[MAX_PLAYERS], FILE *logFile,
  ...
  if (transactionFile != NULL) {
    if (processTransactionFile(game, fixedSeats, &handId, &player0Seat, rng,
                               errorInfo, totalValue, &state,
                               transactionFile) < 0) {
      /* error messages already handled in function */
  ...
  while (1) {
    ...
    while (!stateFinished(&state.state)) {
      ...
      for (seat = 0; seat < game->numPlayers; ++seat) {
        ...
        if (sendPlayerMessage(game, &state, quiet, seat, seatFD[seat], &t) <
            0) {
          /* error messages already handled in function */
      ...
      if (readPlayerResponse(game, &state, quiet, currentSeat, &sendTime,
                             errorInfo, readBuf[currentSeat], &action,
                             &recvTime) < 0) {
        /* error messages already handled in function */
      ...
      /* log the transaction */
      if (transactionFile != NULL) {
        if (logTransaction(game, &state.state, &action, &sendTime, &recvTime,
                           transactionFile) < 0) {
          /* error messages already handled in function */
    ...
    if (logFile != NULL) {
      if (addToLogFile(game, &state.state, value, player0Seat, seatName,
                       logFile) < 0) {
        /* error messages already handled in function */
    ...
    for (seat = 0; seat < game->numPlayers; ++seat) {
      state.viewingPlayer = seatToPlayer(game, player0Seat, seat);
      if (sendPlayerMessage(game, &state, quiet, seat, seatFD[seat], &t) < 0) {
        /* error messages already handled in function */
    ...
    if (!quiet) {
      if (handId % 100 == 0) {
        for (seat = 0; seat < game->numPlayers; ++seat) {
          fprintf(stderr,
                  "Seconds cumulatively spent in match for seat %i: "
                  "%i\n",
                  seat, (int)(errorInfo->usedMatchMicros[seat] / 1000000));
        }
      }
    }
...
int main(int argc, char **argv) {
  ...
  static struct option longOptions[] = {{"t_response", 1, 0, 0},
                                        {"t_hand", 1, 0, 0},
                                        {"t_per_hand", 1, 0, 0},
                                        {"start_timeout", 1, 0, 0},
                                        {0, 0, 0, 0}};
        ...
        switch (longOpt) {
          ...
          case 2:
            ...
            if (sscanf(optarg, "%" SCNu64, &maxUsedPerHandMicros) < 1) {
              fprintf(
                  stderr,
                  "ERROR: could not get average player hand timeout from %s\n",
                  optarg);
              exit(EXIT_FAILURE);
            }
  ...
  printInitialMessage(argv[optind], argv[optind + 1], numHands, seed,
                      &errorInfo, logFile);
  ...
  for (i = 0; i < game->numPlayers; ++i) {
    ...
    v = 1;
    setsockopt(seatFD[i], IPPROTO_TCP, TCP_NODELAY, (char *)&v, sizeof(int));
