...
static enum ActionType charToAction[256] = {
    /* 0x0X */
    a_invalid, a_invalid, a_invalid, a_invalid, a_invalid, a_invalid, a_invalid,
    a_invalid, a_invalid, a_invalid, a_invalid, a_invalid, a_invalid, a_invalid,
    a_invalid, a_invalid,
    /* 0x1X */
    a_invalid, a_invalid, a_invalid, a_invalid, a_invalid, a_invalid, a_invalid,
    ...
    a_invalid, a_invalid,
    /* 0x4X */
    a_invalid, a_invalid, a_raise, a_call, a_invalid, a_invalid, a_fold,
    a_invalid, a_invalid, a_invalid, a_invalid, a_call, a_invalid, a_invalid,
    a_invalid, a_invalid,
    /* 0x5X */
    a_invalid, a_invalid, a_raise, a_invalid, a_invalid, a_invalid, a_invalid,
    a_invalid, a_invalid, a_invalid, a_invalid, a_invalid, a_invalid, a_invalid,
    a_invalid, a_invalid,
    /* 0x6X */
    a_invalid, a_invalid, a_raise, a_call, a_invalid, a_invalid, a_fold,
...
static int consumeSpaces(const char *string, int consumeEqual) {
  ...
  for (i = 0; string[i] != 0 &&
              (isspace(string[i]) || (consumeEqual && string[i] == '='));
       ++i) {
  }
...
static int readItems(const char *itemFormat, const int numItems,
                     const char *string, const int consumeEqual, void *items,
                     const size_t itemSize, int *charsConsumed) {
  int i, c, r;
  char *fmt;
...
Game *readGame(FILE *file) {
  ...
  while (fgets(line, MAX_LINE_LEN, file)) {
    ...
    if (!strncasecmp(line, "end gamedef", 11)) {
      ...
      stackRead =
          readItems("%" SCNd32, MAX_PLAYERS, &line[5], 1, game->stack, 4, &c);
    } else if (!strncasecmp(line, "blind", 5)) {
      blindRead =
          readItems("%" SCNd32, MAX_PLAYERS, &line[5], 1, game->blind, 4, &c);
    } else if (!strncasecmp(line, "raisesize", 9)) {
      raiseSizeRead = readItems("%" SCNd32, MAX_PLAYERS, &line[9], 1,
                                game->raiseSize, 4, &c);
      ...
      readItems("%" SCNu8, MAX_ROUNDS, &line[11], 1, game->firstPlayer, 1, &c);
    } else if (!strncasecmp(line, "maxraises", 9)) {
      readItems("%" SCNu8, MAX_ROUNDS, &line[9], 1, game->maxRaises, 1, &c);
      ...
      boardCardsRead = readItems("%" SCNu8, MAX_ROUNDS, &line[13], 1,
                                 game->numBoardCards, 1, &c);
    }
  }
  ...
  if (boardCardsRead < game->numRounds) {
    fprintf(stderr,
            "only read %" PRIu8 " board card numbers, need %" PRIu8 "\n",
...
uint8_t currentPlayer(const Game *game, const State *state) {
  ...
  return nextPlayer(game, state,
                    game->firstPlayer[state->round] + game->numPlayers - 1);
}
...
static int statesEqualCommon(const Game *game, const State *a, const State *b) {
  int r, i, t;
...
int raiseIsValid(const Game *game, const State *curState, int32_t *minSize,
                 int32_t *maxSize) {
  int p;
  ...
  if (curState->numActions[curState->round] + game->numPlayers >
    ...
    fprintf(stderr,
            "WARNING: #actions in round is too close to MAX_NUM_ACTIONS, "
            "forcing call/fold\n");
    return 0;
  }
...
int isValidAction(const Game *game, const State *curState, const int tryFixing,
                  Action *action) {
  int min, max, p;
...
static int printBetting(const Game *game, const State *state, const int maxLen,
                        char *string) {
  int i, a, c, r;
...
static int readHoleCards(const char *string, const Game *game, State *state) {
  int p, c, r, num;
  ...
  for (p = 0; p < game->numPlayers; ++p) {
    ...
    num = readCards(&string[c], game->numHoleCards, state->holeCards[p], &r);
    if (num == 0) {
...
static int printAllHoleCards(const Game *game, const State *state,
  ...
  for (p = 0; p < game->numPlayers; ++p) {
    ...
    r = printCards(game->numHoleCards, state->holeCards[p], maxLen - c,
                   &string[c]);
    if (r < 0) {
      return -1;
    }
...
static int printPlayerHoleCards(const Game *game, const State *state,
                                const uint8_t player, const int maxLen,
                                char *string) {
  int p, c, r;
...
static int readBoardCards(const char *string, const Game *game, State *state) {
  int i, c, r;
...
static int printBoardCards(const Game *game, const State *state,
  ...
  for (i = 0; i <= state->round; ++i) {
    ...
    r = printCards(game->numBoardCards[i], &state->boardCards[bcStart(game, i)],
                   maxLen - c, &string[c]);
    if (r < 0) {
      return -1;
    }
...
static int readStateCommon(const char *string, const Game *game, State *state) {
  uint32_t handId;
  int c, r;
...
int readMatchState(const char *string, const Game *game, MatchState *state) {
  ...
  if (sscanf(string, "MATCHSTATE:%" SCNu8 "%n", &state->viewingPlayer, &c) <
          1 ||
...
int printState(const Game *game, const State *state, const int maxLen,
               char *string) {
  int c, r;
...
int printMatchState(const Game *game, const MatchState *state, const int maxLen,
                    char *string) {
  int c, r;
...
int printAction(const Game *game, const Action *action, const int maxLen,
                char *string) {
  int c, r;
...
int readCards(const char *string, const int maxCards, uint8_t *cards,
              int *charsConsumed) {
  int i, c, r;
...
int printCards(const int numCards, const uint8_t *cards, const int maxLen,
               char *string) {
  int i, c, r;
